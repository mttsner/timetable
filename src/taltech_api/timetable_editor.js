...
function removeEmptyWeekdays(scheduleJson) {
    return {
 ...
 */
function removeStudentGroup(scheduleJson, studentGroupId) {
 ...
 */
function combineSchedules(scheduleJson, otherScheduleJson) {
    ...
    weekDays = weekDays.map((weekDay, index) => {
        let newRows = [];
        if(scheduleJson["weekDays"].some((day) => day["dow"] === weekDay["dow"])) {
            newRows = newRows.concat(scheduleJson["weekDays"].find((day) => day["dow"] === weekDay["dow"])["rows"]);
        }

        if(otherScheduleJson["weekDays"].some((day) => day["dow"] === weekDay["dow"])) {
            newRows = newRows.concat(otherScheduleJson["weekDays"].find((day) => day["dow"] === weekDay["dow"])["rows"]);
        }
